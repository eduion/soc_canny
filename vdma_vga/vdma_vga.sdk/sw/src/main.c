	...
	*(volatile unsigned int *)(VDMA_ADDR + 0x50) = VSIZE; //VSIZE
	...
	while(1){
		...
		if(state == 1 && flag==0)
		{
			...
			*(volatile unsigned int *)(VDMA_ADDR + 0x28) = 1; // Frame 1 礶
			usleep(50000); // 单翴翴丁絋玂 frame cycle 筁近
			// 单秙秨
		...
		{
			printf("Button Pressed\n");
			*(volatile unsigned int *)(VDMA_ADDR + 0x28) = 0; // Frame 0 礶
			usleep(50000); // 单翴翴丁絋玂 frame cycle 筁近
			// 单秙秨
			while(debounce_button(ps_button) == 1);
			flag = 0;
